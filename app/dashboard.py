...
# Fix for ValueError: Drop target so it matches Scaler features
...
# Fix for TypeError/ValueError:  gets scalar risk prob from 2D array
risk_prob_val = model.predict_proba(X_input_scaled)
risk_prob = float(risk_prob_val[1])
...
shap_values = explainer(pd.DataFrame(X_input_scaled, columns=X_input.columns))

# Fix for SHAP plot: [0, :, 1] selects the 'Risk' class explanation for the first student
